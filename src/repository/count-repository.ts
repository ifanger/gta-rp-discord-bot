...
export default class CountRepository {
  ...
  incrementCount = async () => this.getDatabase().update("count", (current: number) => current + 1).write();

  getCount = async () => this.getDatabase().get("count").value();

  resetCount = async () => this.getDatabase().update("count", (current: number) => 0).write();
}
