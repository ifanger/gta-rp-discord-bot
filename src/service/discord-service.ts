...
import { Message, TextChannel, GuildChannelManager, RoleManager } from "discord.js";
...
export default class DiscordService {
  ...
  createPrivateChannel = async (discordId: string, channelManager: GuildChannelManager, roleManager: RoleManager, count: number) => {
    const channel = await channelManager.create(
      `whitelist-${count}`,
      { type: 'text', userLimit: 2, parent: config.server.category }
    );
  ...
  askForId = async (channel: TextChannel, memberId: string) => {
    const question = await channel.send('*Qual o seu ID em nosso servidor?*\n\n:speech_left: _Seu ID foi exibido ao tentar conectar em nosso servidor, no FiveM._');
    const response = await channel.awaitMessages((message) => (message.author.id === memberId) && !isNaN(message.content), { max: 1, errors: ['time'], time: 60000 });
  ...
  askForName = async (channel: TextChannel, memberId: string) => {
    const question = await channel.send('*Qual o seu nome (com sobrenome) na nossa cidade?*\n\n:speech_left: _Seu nome in-game._');
    const response = await channel.awaitMessages((message) => (message.author.id === memberId), { max: 1, errors: ['time'], time: 60000 });
